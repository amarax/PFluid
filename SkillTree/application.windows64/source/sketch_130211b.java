...
public class sketch_130211b extends PApplet {
...
Global_Float abilityGapSize = new Global_Float( 0.3f );
...
class TSW_AbilityTree_Generated extends TSW_AbilityTree
{
  ...
  protected TSW_AbilityBranch addNewBranch( String aName, int aColor, TSW_AbilityBranch aParentBranch )
  {
  ...
  protected TSW_Ability addNewAbility( String aName, int aPoints, int aLockedColor, int aUnlockedColor, TSW_AbilityBranch aParentBranch )
  {
...
class TSW_UIControl_AbilityTree extends UIControl
{
  ...
  public void update()
  {
    ...
    float tCenterX = rect.x + rect.width / 2.0f;
    float tCenterY = rect.y + rect.height / 2.0f;  
  ...
  public void draw()
  {
    ...
    if( false ) // For debugging
    {
      ...
      ellipseMode( RADIUS );
      ellipse( tCenterX, tCenterY, rect.width / 2.0f, rect.height / 2.0f );    
    }
...
//      arc( tCenterX, tCenterY, tGlowRadius, tGlowRadius, PI + HALF_PI + selectedAbilityNode.linkedControl.damper_startAngle.getValue(), PI + HALF_PI + selectedAbilityNode.linkedControl.damper_endAngle.getValue(), OPEN );     
  ...
  private void updateChildNodeDimensions( TSW_AbilityNode aNode, int aLevel, float aStartAngle, float aEndAngle )
  {
    ...
    for( TSW_AbilityNode iNode : aNode.getChildNodes() )
    {
      ...
      tControl.cDistanceFromCenter = ( aLevel * 1.0f / abilityTree.cLevels ) * ( outerRingSize.value - ringThickness.value - innerRingSize.value ) + innerRingSize.value;
...
class TSW_UIControl_AbilityNode extends UIControl
{
  ...
  public void setup( TSW_UIControl_AbilityTree aParentTreeControl )
  {
    ...
    damper_startAngle = new DampingHelper_Float( 0.05f, 0 );
    damper_endAngle = new DampingHelper_Float( 0.05f, 0 );
  }
  ...
  public void draw()
  {
    ...
    float tCenterX = parentTreeControl.rect.x + parentTreeControl.rect.width / 2.0f;
    float tCenterY = parentTreeControl.rect.y + parentTreeControl.rect.height / 2.0f;  
    ...
    if( tEndAngle - tStartAngle >= 1 / tDistanceFromCenter )
    {
      ...
      if( parentTreeControl.selectedAbilityNode != null )
      {
        ...
        {
          stroke( hue( linkedNode.nodeColor ), saturation( linkedNode.nodeColor ) * 0.2f, 1.0f, alpha( linkedNode.nodeColor ) * 0.15f );
  ...
  public boolean isMouseIn()
  {
    ...
    {
      Vector2 tCenter = new Vector2( parentTreeControl.rect.x + parentTreeControl.rect.width / 2.0f, parentTreeControl.rect.y + parentTreeControl.rect.height / 2.0f );  
...
class UIControl_Switch extends UIControl
{
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "sketch_130211b" };
    if (passedArgs != null) {
